  ...
  $.getJSON(
    ...
    function(data) {
      $.each(data, function(key, item) {
        $(".sold-itens").append("<div class='sold-itens__card" + key + "'>");
        $(".sold-itens__card" + key).append(
          "<img src='" + item.productImage + "' alt='' width='100%'>"
        );
        $(".sold-itens__card" + key).append("<p>" + item.productName + "</p>");
        $(".sold-itens__card" + key).append(
          "<div class='sold-itens__card--rating'>"
        );

        var rating = item.productRating / 10;

        for (var i = 0; i < rating; i++) {
          $(".sold-itens__card" + key)
            .find(".sold-itens__card--rating")
            .append(
        ...
        $(".sold-itens__card" + key).append(
          "<h3><b>por R$ " + item.productBestPrice.toFixed(2) + "</b></h3>"
        );
        $(".sold-itens__card" + key).append(
          "<span>ou por " +
            item.productInstallments +
            "x de R$ " +
            item.productBestPrice.toFixed(2) +
            "</span>"
        );

        $(".sold-itens__card" + key).append("<a href='#'>Comprar</a>");
      });
    }
  );
